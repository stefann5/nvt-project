"""
Load Test Runner for Warehouse Management System
...
def run_scenario(scenario_key, host, scenario_config):
    ...
    csv_prefix = os.path.join(RESULTS_DIR, f"{scenario_key}_{timestamp}")
    html_report = os.path.join(RESULTS_DIR, f"{scenario_key}_{timestamp}_report.html")
    log_file = os.path.join(RESULTS_DIR, f"{scenario_key}_{timestamp}.log")
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"\n✓ Scenario '{scenario_key}' completed successfully in {elapsed/60:.1f} minutes")
            print(f"  Results saved to: {csv_prefix}*")
            print(f"  HTML Report: {html_report}")
        else:
            print(f"\n✗ Scenario '{scenario_key}' failed with return code {result.returncode}")
        
        return result.returncode == 0
    except Exception as e:
        print(f"\n✗ Error running scenario '{scenario_key}': {e}")
        return False
...
def main():
    parser = argparse.ArgumentParser(
        description='Run load tests for Warehouse Management System',
...
Examples:
  ...
  python run_load_tests.py --scenario all
    ...
    print("=" * 70)
    print("WAREHOUSE MANAGEMENT LOAD TEST SUITE")
    print("=" * 70)
